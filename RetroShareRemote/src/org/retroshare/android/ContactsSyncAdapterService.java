...
public class ContactsSyncAdapterService extends Service
{
    ...
    private static void addContact(Account account, String name, String username) {
        ...
        builder.withValue(ContactsContract.Data.MIMETYPE, "retroshare.android.cursor.item/vnd.org.retroshare.android.sync.profile");
        ...
        try {
            while (c.moveToNext()) {
                if (!c.isNull(1)) {
                    ...
                    if (mimeType.equals("retroshare.android.cursor.item/vnd.org.retroshare.android.sync.profile")) {
	...
	private static void performSync(Context context, Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) throws OperationCanceledException
	{
        ...
        Log.i(TAG, "SINCAAAAAAAAAAAAAAAAAAAAAAAAAAAA------------------------------------" + account.toString());
        Log.v(TAG, "=========================PEERSETDATA\n");
        ...
        ArrayList<ContentProviderOperation> operationList = new ArrayList<ContentProviderOperation>();
        String pname="canedidio";
        Log.v(TAG, "=========================" + pname + "\n");
        try {
            // If we don't have any contacts, create one. Otherwise, set a
            // status message
            if (localContacts.get(pname) == null) {
                addContact(account, pname+"@Location", pname);
            } else {
                if (localContacts.get(pname).photo_timestamp == null || System.currentTimeMillis() > (localContacts.get(pname).photo_timestamp + 604800000L)) {
                    ...
                    Bitmap icon = BitmapFactory.decodeResource(context.getResources(), R.drawable.retrosharelogo2);
                    icon.compress(Bitmap.CompressFormat.PNG, 0, stream);
                    updateContactPhoto(operationList, localContacts.get(pname).raw_id, stream.toByteArray());
                }
                updateContactStatus(operationList, localContacts.get(pname).raw_id, "Mille cose, Test duro");
            }
